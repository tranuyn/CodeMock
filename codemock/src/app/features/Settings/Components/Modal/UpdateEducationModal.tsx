...
const formSchema = z.object({
  educationBackground: z.array(educationEntrySchema),
});

const UpdateEducationModal = ({ user, onClose }: SkillFormProp) => {
  const methods = useForm<{
    educationBackground: experience_detail[];
  }>({
    defaultValues: {
      educationBackground: user.educationBackground?.length
  ...
  const nothingChanged =
    JSON.stringify(watch("educationBackground")) ===
      JSON.stringify(
        user.educationBackground || [
          {
            position: "",
  ...
  const SaveeducationBackground = async () => {
    setLoading(true);
    const updatedEducation = [...getValues("educationBackground")].map(
      (entry) => ({
        ...entry,
          ...
          >
            ...
            >
              <InputComponent
                ...
                name={`educationBackground.${index}.position`}
                register={register}
                inputStyle={styles.customInput}
              />
              <InputComponent
                title="Nơi làm việc"
                name={`educationBackground.${index}.work_space`}
                register={register}
                inputStyle={styles.customInput}
              />
              <div className={styles.flexRow} style={{ gap: "20px" }}>
                <InputComponent
                  title="Năm bắt đầu"
                  allowNumbers={true}
                  name={`educationBackground.${index}.yearStart`}
                  register={register}
                  inputStyle={styles.customInput}
                />
                <InputComponent
                  title="Năm kết thúc"
                  allowNumbers={true}
                  name={`educationBackground.${index}.yearEnd`}
                  register={register}
                  inputStyle={styles.customInput}
                />
              </div>
