...
const AppBar: React.FC<AppBarProps> = ({ isHomePage, isShow = true }) => {
  ...
  const adminRoutes = [
    {
      name: "Xem lại và đánh giá",
      path: "/viewAndFeedback",
  ...
  const renderDrawerRouteItem = (route: {
    name: string;
    path: string;
      ...
      <MuiAppBar
        ...
        sx={{
          background: isHomePage
            ? "transparent"
            : "linear-gradient(45deg, #501794 30%, #3E70A1 90%)",
      ...
      >
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            ...
            >
              {[...publicRoutes, ...authenticatedRoutes, ...adminRoutes].map(
                (route) => (
                  <Button
                    key={route.name}
                    component={Link}
